 ...
 * 우리는 AuthenticationFilureHandler를 구현해야 하지만 기존에 구현이 되었는 SimpleUrlAuthenticationFailureHandler를 상속받아
...
public class AuthFailHandler extends SimpleUrlAuthenticationFailureHandler {
    /*
     * 설명 : onAuthenticationFailure 메소드가 호출될 defaultFailureUrl인 경우 리다이렉션을 수행하는 AuthenticationFailUreHandler이다.
     *       속성이 설정되어 있지 않는 경우 실패를 일으킨 AuthenticationException의 오류 메세지와 함게 클라이언트에게 401 오류를 응답한다.
     * */
