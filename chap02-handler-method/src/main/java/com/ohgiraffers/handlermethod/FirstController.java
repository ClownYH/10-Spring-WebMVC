...
public class FirstController {
    ...
    * 위의 first는 template 하위 디렉토리를 의미한다.
    * */
    @GetMapping("regist")
    public void regist(){}
    ...
    * */
    ...
    public String registMenu(Model model, WebRequest request){
        /* WebRequest 객체의 getParameter 등의 메소드를 통해 클라이언트로부터 전달된 파라미터를 가져올 수 있다. */
    ...
    public String modifyMenuPrice(Model model, @RequestParam(required = false) String modifyName, @RequestParam(defaultValue = "0") int modifyPrice){
        // @RequestParam은 매개변수명과 요청값의 이름이 다를 때 사용되며, 동일할 때 발생할 오류의 대응과 프론트엔드와 백엔드의 서로 다른 유지보수 측면을 대응하기 위해 사용한다.
    ...
    * */
    ...
    public String modifyAll(Model model, @RequestParam Map<String, String> parameters){
        ...
        String message = "메뉴의 이름은 " + modifyMenu + "(으)로, 가격을 " + modifyPrice + "원으로 변경하였습니다.";
        System.out.println(message);
    ...
    *
    * 경우에 따라 폼에서 입력한 값을 다음 화면으로 전달해야 하는 경우가 발생하는데 이 때 유용하게 사용할 수 있다.
    ...
    *
    * 주로 restAPI 작성 시 많이 사용되며, 일반적으로 form 전송을 할 때는 거의 사용하지 않는다.
    *
    * 추가적으로 헤더에 대한 정보도
